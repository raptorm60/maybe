...
class Settings::HostingsControllerTest < ActionDispatch::IntegrationTest
  ...
  setup do
    ...
    @provider = mock
    Provider::Registry.stubs(:get_provider).with(:finnhub).returns(@provider)
  ...
  test "can update settings when self hosting is enabled" do
    with_self_hosting do
      patch settings_hosting_url, params: { setting: { finnhub_api_key: "1234567890" } }

      assert_equal "1234567890", Setting.finnhub_api_key
    end
  end
